...
if st.session_state.onboarded:
    ...
    with st.sidebar:
        st.header("Settings")
        st.session_state.user_id = st.number_input("Student ID (1-10)", min_value=1, max_value=10, value=st.session_state.user_id)
    ...
    with tab3:
        # This entire multi-screen logic is now nested within the CGPA Predictor Tab
        if st.session_state.page == 'Screen 1':
            st.header("Step 1: Previous Semester Courses")
            ...
            with st.form("prev_form"):
                ...
                for i in range(5):
                    ...
                    cols = st.columns(3)
                    course_id = cols[0].text_input(f"Course ID", key=f"prev_course_id_{i}", placeholder="e.g., MATH101")
                    grade = cols[1].number_input(f"Grade (0-100)", min_value=0.0, max_value=100.0, step=0.1, key=f"prev_grade_{i}")
                    course_units = cols[2].number_input(f"Course Units", min_value=1, max_value=6, step=1, key=f"prev_units_{i}")
                    
                    cols2 = st.columns(3)
                    study_hours = cols2[0].number_input(f"Study Hours/Week", min_value=0.0, step=0.1, key=f"prev_hours_{i}")
                    course_difficulty = cols2[1].number_input(f"Difficulty (1-5)", min_value=1, max_value=5, step=1, key=f"prev_diff_{i}")
                    attendance = cols2[2].number_input(f"Attendance (%)", min_value=0.0, max_value=100.0, step=0.1, key=f"prev_att_{i}")
                    
                    prev_courses.append({
                        'user_id': st.session_state.user_id, 'semester': 'Previous', 'course_id': course_id,
                        'grade': grade, 'study_hours': study_hours, 'course_units': course_units,
                        'course_difficulty': course_difficulty, 'attendance': attendance
                    })

                st.markdown("---")
                cols3 = st.columns(3)
                semester_gpa = cols3[0].number_input("Last Semester GPA (0-5)", min_value=0.0, max_value=5.0, step=0.01)
                current_cgpa = cols3[1].number_input("Overall CGPA (0-5)", min_value=0.0, max_value=5.0, step=0.01)
                learning_style = cols3[2].selectbox("Learning Style", ["Visual", "Auditory", "Kinesthetic"])
            ...
            st.header("Step 2: Current Semester Courses")
            ...
            with st.form("curr_form"):
                ...
                for i in range(5):
                    st.subheader(f"Course {i+1}")
                    cols = st.columns(3)
                    course_id = cols[0].text_input(f"Course ID", key=f"curr_course_id_{i}", placeholder="e.g., STATS101")
                    course_units = cols[1].number_input(f"Course Units", min_value=1, max_value=6, step=1, key=f"curr_units_{i}")
                    course_difficulty = cols[2].number_input(f"Difficulty (1-5)", min_value=1, max_value=5, step=1, key=f"curr_diff_{i}")
                    curr_courses.append({
                        'user_id': st.session_state.user_id, 'semester': 'Current', 'course_id': course_id,
                        'course_units': course_units, 'course_difficulty': course_difficulty,
            ...
            if not X.empty and not y.empty:
                ...
                # FIX: The 'grade' column is needed for prediction. Create it from the estimated value.
                user_curr['grade'] = avg_grade - (user_curr['course_difficulty'] - user_prev['course_difficulty'].mean()) * 5
                ...
                X_curr = user_curr[[col for col in features if col in user_curr.columns]]
                ...
                weak_course = user_prev.loc[user_prev['grade'].idxmin()]
                high_difficulty = user_prev[user_prev['course_difficulty'] >= 4]['course_id'].tolist()
                ...
                st.write(f"- Weakest Course: {weak_course['course_id']} (Grade: {weak_course['grade']:.1f}%)")
                if high_difficulty:
                ...
                if weak_course['grade'] < 70:
                    st.write(f"- **Possible Cause**: Poor performance in {weak_course['course_id']} significantly affected GPA.")
                ...
                st.dataframe(user_curr[['course_id', 'course_units', 'course_difficulty', 'allocated_hours']].style.format({'allocated_hours': '{:.1f} hours/week'}))
                ...
                if api_key:
                    try:
                        ...
                        prompt = f"""
                        As an expert academic advisor, provide personalized recommendations for student {st.session_state.user_id}.
                        
                        **Previous Semester Analysis:**
                        - Last Semester GPA: {user_prev['semester_gpa'].iloc[0]:.2f}
                        - Overall CGPA: {user_prev['current_cgpa'].iloc[0]:.2f}
                        - Weakest Course: {weak_course['course_id']} (Grade: {weak_course['grade']:.1f}%)
                        - Past Study Habits: {avg_hours:.1f} hours/week, {avg_attendance:.1f}% attendance.
                        - Learning Style: {learning_style}

                        **Current Semester Outlook:**
                        - Courses: {', '.join(f"{row['course_id']} ({row['course_units']} units, difficulty: {row['course_difficulty']})" for _, row in user_curr.iterrows())}.
                        - Predicted Semester GPA: {predicted_gpa:.2f}.

                        **Task:**
                        Generate a concise, actionable 1-week study plan. Suggest specific learning resources (e.g., YouTube channels, websites) and strategies to address past weaknesses (especially for {weak_course['course_id']}). Tailor the advice to a {learning_style} learner. Structure your response with clear headings.
                        """
                    ...
                    st.warning("Please set the API_KEY environment variable to get AI-powered recommendations.")
                ...
                st.warning("No previous data found. Please go back and enter your data.")
